...
TOKEN = os.environ.get("DISCORD_BOT_TOKEN_WH40K") or os.environ["DISCORD_TOKEN"]
BASE = "https://discord.com/api/v10"
GUILD = os.environ.get("FILM_GUILD_ID", "1552514539344363610")
S = requests.Session()
...
def channel_messages(cid, limit=100):
    ...
    while True:
        ...
        if not batch:
            break
        out.extend(batch)
        if len(batch) < 100 or len(out) >= limit:
            break
        before = batch[-1]["id"]
    return out
